...
export const isPublicKeyPair = (input: string) =>
  !input.startsWith('/') && !input.endsWith('0=');

export const extractPlusCode = (pubKey: string) => {
  const match = pubKey.match(/^\/\/([^0=]+)/);

  if (match) {
    const extract = match[1];
    return { isValid: OLC.isFull(extract), plusCode: extract };
  } else {
    return null;
  }
};
